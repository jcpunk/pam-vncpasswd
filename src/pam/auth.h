 ...
 * 1. If file_override: use it directly (skip home directory lookup)
 *    Otherwise: look up home directory via getpwnam_r, build canonical path
 *    via build_vnc_passwd_path()
 * 2. Open and validate the password file (TOCTOU-safe)
 * 3. Read stored hash from file
 * 4. mlock password buffer in RAM (non-fatal if it fails)
