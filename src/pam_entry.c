 ...
 * to the core logic in pam_vncpasswd.c.
 *
 * WHY SEPARATE FROM pam_vncpasswd.c:
 * The core authentication logic (pam_vncpasswd.c) has no PAM dependency,
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags,
  ...
  if (pam_get_user(pamh, &username, NULL) != PAM_SUCCESS || !username) {
    pam_syslog(pamh, LOG_ERR, "pam_vncpasswd: could not get username");
    return PAM_AUTH_ERR;
  }

  if (pam_get_authtok(pamh, PAM_AUTHTOK, &authtok, NULL) != PAM_SUCCESS
      || !authtok) {
    pam_syslog(pamh, LOG_ERR, "pam_vncpasswd: could not get password");
    return PAM_AUTH_ERR;
  }
